# -----------------------------------------------
# SCRIPT TO PARSE THE CHIA DATASET FOR NER TASKS
# Author: Matthias Guetebier
# -----------------------------------------------
...
def get_entity_presence_matrix(dataset: Dataset, label_column: str = "labels"):
    ...
    Returns:
        y_matrix (np.array): binary matrix of shape (n_samples, n_entity_types)
...
def load_chia_dataset(data_dir: str, model_checkpoint: str):
    """
    ...
    logger.info(f"Found {len(txt_files)} files in {data_dir}. Starting processing...")
...
def split_and_save_dataset_iterative(
    dataset: Dataset,
    output_dir: str,
    seed: int = 42,
...
if __name__ == "__main__":
    ...
    if not os.path.exists(DATA_DIR):
        ...
        # %%
        if len(dataset) > 0:
            # (3) stratify, split and save processed dataset
            split_and_save_dataset_iterative(dataset, OUTPUT_DIR, seed=RANDOM_SEED)
